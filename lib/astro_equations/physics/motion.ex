defmodule AstroEquations.Physics.Motion do
  ...
  @typedoc "Velocity in metres per second (m/s)."
  @type velocity :: float()

  @typedoc "Angular velocity in radians per second (rad/s)."
  @type angular_velocity :: float()

  @typedoc "Moment of inertia in kg·m²."
  @type moment_of_inertia :: float()

  @typedoc "Time in seconds (s). Non-negative."
  @type time :: float()
  ...
  def velocity(displacement, time), do: displacement / time

  @doc "Average acceleration: a = Δv/Δt."

  @spec acceleration(number, number) :: float
  def acceleration(delta_v, time), do: delta_v / time

  @doc "SUVAT final velocity: v = u + at."

  @spec final_velocity(number, number, number) :: number()
  def final_velocity(u, a, t), do: u + a * t

  @doc "SUVAT displacement: s = ut + ½at²."

  @spec displacement_suvat(number, number, number) :: float
  def displacement_suvat(u, t, a), do: u * t + 0.5 * a * t * t
  ...
  def displacement_average(u, v, t), do: (u + v) * t / 2
  ...
  def newtons_second_law(mass, accel) when is_positive(mass), do: mass * accel
  ...
  def acceleration_from_force(net_force, mass) when is_positive(mass), do: net_force / mass
  ...
  def kinetic_energy(mass, velocity) when is_positive(mass) and is_non_negative(velocity),
    do: 0.5 * mass * :math.pow(velocity, 2)
  ...
  def total_kinetic_energy(mass, velocity, moment_of_inertia, angular_velocity)
  ...
  def centripetal_force(mass, velocity, radius) when is_positive(mass) do
  ...
  def orbital_speed(central_mass, radius)
      when is_positive(central_mass) and is_positive(radius) do
  ...
  def angular_velocity(angular_displacement, time), do: angular_displacement / time

  @doc "Average angular acceleration: α = Δomega/Δt."

  @spec angular_acceleration(number, number) :: float
  def angular_acceleration(delta_omega, time), do: delta_omega / time

  @doc "Tangential (linear) velocity of a rotating body: v_t = omegar."

  @spec tangential_velocity(number, number) :: number()
  def tangential_velocity(omega, radius) when is_positive(radius), do: omega * radius

  @doc "Tangential acceleration: a_t = αr."

  @spec tangential_acceleration(number, number) :: number()
  def tangential_acceleration(alpha, radius) when is_positive(radius), do: alpha * radius

  @doc "Angular momentum of a rigid body: L = Iomega."

  @spec angular_momentum(number, number) :: number()
  ...
  @spec rotational_newtons_law(number, number) :: number()
  ...
  @spec angular_impulse(number, number) :: number()
  def angular_impulse(torque, time), do: torque * time
  ...
  def point_mass_moment_of_inertia(mass, radius) when is_positive(mass) do
    mass * :math.pow(radius, 2)
  end

  @doc "Moment of inertia of a system of point masses: I = Σ mᵢrᵢ²."

  @spec multiple_point_masses_moment_of_inertia([number], [number]) :: number()
  ...
  def thin_disk_moment_of_inertia(mass, radius) when is_positive(mass) do
  ...
  def thin_loop_moment_of_inertia(mass, radius) when is_positive(mass) do
    mass * :math.pow(radius, 2)
  end
  ...
  def thin_rod_center_moment_of_inertia(mass, length) when is_positive(mass) do
  ...
  def thin_rod_end_moment_of_inertia(mass, length) when is_positive(mass) do
  ...
  def solid_sphere_moment_of_inertia(mass, radius) when is_positive(mass) do
  ...
  def hollow_sphere_moment_of_inertia(mass, radius) when is_positive(mass) do
  ...
  def hollow_cylinder_moment_of_inertia(mass, inner_radius, outer_radius)
      when is_positive(mass) do
  ...
  def parallel_axis(i_cm, mass, d) when is_positive(mass) do
  ...
  @spec horizontal_displacement(number, number) :: number()
  def horizontal_displacement(initial_vx, time), do: initial_vx * time
  ...
  def spring_angular_frequency(k, mass) when is_positive(k) and is_positive(mass),
    do: :math.sqrt(k / mass)
  ...
  @spec lagrangian(number, number) :: number()
  ...
  def poisson_bracket(f, g, q, p, h \\ 1.0e-6) do
    df_dq = partial_q(f, q, p, h)
    df_dp = partial_p(f, q, p, h)
    dg_dq = partial_q(g, q, p, h)
    dg_dp = partial_p(g, q, p, h)
    df_dq * dg_dp - df_dp * dg_dq
  end

  defp partial_q(f, q, p, h), do: (f.(q + h, p) - f.(q - h, p)) / (2 * h)
  defp partial_p(f, q, p, h), do: (f.(q, p + h) - f.(q, p - h)) / (2 * h)
  ...
  def keplers_third_law(a, central_mass) when is_positive(a) and is_positive(central_mass) do
  ...
  def vis_viva(central_mass, r, a)
      when is_positive(central_mass) and is_positive(r) and is_positive(a) do
